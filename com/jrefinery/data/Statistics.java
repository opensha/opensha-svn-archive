...
public class Statistics {
    ...
    public static double[] getLinearFit(Number[] x_data, Number[] y_data) {
        ...
        if(x_data.length != y_data.length) {
            throw new IllegalArgumentException("Statistics.getLinearFit(...): array lengths "
                                               +"must be equal.");
        }

        double[] result = new double[2];
        result[1] = getSlope(x_data, y_data);  // slope
        result[0] = getAverage(y_data) - result[1] * getAverage(x_data);  // intercept
    ...
    public static double getSlope(Number[] x_data, Number[] y_data) {
        ...
        // check arguments...
        if(x_data.length != y_data.length) {
            throw new IllegalArgumentException("Statistics.getSlope(...)"
    ...
    public static double getCorrelation(Number[] data1, Number[] data2) {
        ...
        if(data1.length != data2.length) {
            throw new IllegalArgumentException("Statistics.getCorrelation(...)"
                                              +": array lengths must be equal.");
        }
