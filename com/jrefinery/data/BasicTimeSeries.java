...
public class BasicTimeSeries extends Series {
     ...
     * <P>
     * By default, a daily time series is created.  Use one of the other constructors if you
     ...
     * <P>
     * Descriptions can be specified for the domain and range.  One situation where this is
     * helpful is when generating a chart for the time series - axis labels can be taken from the
     ...
     * <p>
     * Only one time period class can be used within a single series (enforced).  If you add a
     * data item with a Year for the time period, then all subsequent data items must also have
     * a Year for the time period.
     *
    ...
    public TimeSeriesDataPair getDataPair(TimePeriod period) {
        ...
        // check arguments...
        if (period==null) {
            throw new IllegalArgumentException("TimeSeries.getDataPair(...): "
                                               +"null time period not allowed.");
        }
    ...
    /**
     * Returns a collection of time periods in the specified series, but not in this series, and
     * therefore unique to the specified series.
     */
     ...
     *
     * @return The value (null if the time period is not in the series).
     */
    ...
    public void add(TimeSeriesDataPair pair) throws SeriesException {
        ...
        if (!pair.getPeriod().getClass().equals(timePeriodClass)) {
            ...
            message = message + "but the BasicTimeSeries is expecting an instance of "+timePeriodClass.getName()+".";
    ...
    /**
     * Updates (changes) the value for a time period.  Ignores the update if the period does not
     * exist.
     *
    ...
    public void update(TimePeriod period, Number value) throws SeriesException {
        ...
        else throw new SeriesException("TimeSeries.update(TimePeriod, Number): "
    ...
    /**
     * Adds or updates data from one series to another.  Returns another series containing the
     * values that were overwritten.
     */
    public BasicTimeSeries addAndOrUpdate(BasicTimeSeries series) {

        BasicTimeSeries overwritten = new BasicTimeSeries("Overwritten values from: "
                                                          +this.getName());
        ...
        for (int i=0; i<series.getItemCount(); i++) {
            ...
            if (oldPair!=null) {
                try {
                    ...
                    System.err.println("TimeSeries.addAndOrUpdate(series): unable to add data to "
                                       +"overwritten series.");
                }
            }
        }
    ...
    public static void main(String[] args) {
        BasicTimeSeries series = new BasicTimeSeries("Test", com.jrefinery.data.Minute.class);
        series.add(new Day(1, 1, 2002), 5.65);
    }
