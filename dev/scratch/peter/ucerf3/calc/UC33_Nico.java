			...
			for (String dm : dmWts.keySet()) {
				...
				for (Period p : periods) {
					...
					for (NEHRP_TestCity city : cities) {
						...
						HazardCalc hc = HazardCalc.create(wrappedERF, imr, site, p, true);
	...
	private static void buildCityCurveUCERF2w2104() throws IOException {
		...
		for (Period p : periods) {
			...
			for (NEHRP_TestCity city : cities) {
				...
				Site site = new Site(city.location());
				HazardCalc hc = HazardCalc.create(erf, imr, site, p, true);
				ecs.submit(hc);
			}
		...
		for (Period p : periods) {
			...
			for (String branch : branches) {
				...
				for (NEHRP_TestCity city : cities) {
					...
					List<ProbEqkRupture> rups = Lists.newArrayList();
					...
					for (int i=0; i<10; i++) {
						...
						ProbEqkRupture rup = rups.get(idx);
						RuptureSurface surf = rup.getRuptureSurface();
						String line = Math.exp(means.get(idx)) + "\t" + 
									  rup.getMag() + "\t" + 
									  surf.getDistanceJB(city.location()) + "\t" +
									  names.get(idx) + LF;
					...
					// save max
					Double max1 = maxMean.get(city);
					Double max2 = means.get(indices.get(0));
					maxMean.put(city, (max1 == null) ? 
						Math.exp(max2) : 
						Math.max(max1, Math.exp(max2)));					
				}
			}
			
			// write max vals
			File maxOut = new File("tmp/forNico/det/" + p.name() + "maxVals.csv");
			Files.write("city, maxMedian" + LF, maxOut, US_ASCII);
			for (NEHRP_TestCity city : cities) {
				Files.append(city + ", " + maxMean.get(city) + LF, maxOut, US_ASCII);
			}
		}		
