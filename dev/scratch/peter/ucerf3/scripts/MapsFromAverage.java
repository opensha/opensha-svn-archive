...
public class MapsFromAverage {
	...
	public static void main(String[] args) throws IOException {
		if (args.length != 11) {
			System.out
				.println("USAGE: " +
					...
					" <queue> <nodes> <hours> <libDir> <scriptPath> <solCount>" +
		...
		int solCount = Integer.parseInt(args[5]);
		
		List<String> otherArgs = Lists.newArrayList(Iterables.skip(
			Arrays.asList(args), 6));

		writeScript(libDir, hours, nodes, queue, solCount, scriptPath, otherArgs);
	}

	private static void writeScript(String libDir, int hrs,
			int nodes, String queue, int solCount, String scriptPath, 
			List<String> args) {
		try {
			...
			for (int i=0; i < solCount ;i++) {
				classNames.add(UC3_CalcMPJ_MapAverage.class.getName());
				// insert branchID as second arg
				args.add(1, Integer.toString(i));
