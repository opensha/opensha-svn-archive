...
public class CurveUtilsNSHMP14 {
	...
	public static void main(String[] args) throws IOException {
		...
		String srcPath = ROOT + "curves/src/tree-gmm/";
		String outPath = ROOT + "curves/src-reduce/NSHMP14-GMM/";
...
//		}
	...
	private static void runBranchSummaries2(String dir, String flagsPath,
	...
	private static void summarizePE(String srcDir, File flagFile,
		...
		// compute and write mean for each branch choice and tMean with branch removed
	...
	private static void writeLeafData(Map<String, double[]> leafMap, double mean,
			File leafFile) throws IOException {
		...
		for (Entry<String, double[]> leafEntry : leafMap.entrySet()) {
			Files.append(leafEntry.getKey() + "," + leafEntry.getValue()[0] +
				"," + leafEntry.getValue()[1] + LF, leafFile, US_ASCII);
		}
	}
	...
	private static double meanForLeaf(XY_DataSetList curves,
			List<Double> weights, List<Boolean> flags) {
	...
	private static double meanWithoutLeaf(XY_DataSetList curves,
			List<Double> weights, List<Boolean> flags) {
	...
	private static void reorganize(Set<Period> periods,
			String srcPath, String outPath, String locPath)
			throws IOException {
		...
		for (int i = 0; i < branchIDs.size(); i++) {
			...
			for (Period p : periods) {
				...
				for (Entry<String, Double> gmmEntry : GMM_ID_WT_MAP.entrySet()) {
					...
					File curveFile = new File(srcPath + "NSHMP14-" + gmmID + S +
						branchID + S + p + S + CURVE_FILE);
					...
					for (String line : Iterables.skip(lines, 1)) {
						...
						int idx = GMM_ID_WT_MAP.size() * i + gmmCount;
						double wt = branchWT * gmmWT;

						String curveStr = idx + "," + wt + "," + curve + LF;
						File cOut = new File(outPath + p + S + loc + S + CURVE_FILE);
						Files.append(curveStr, cOut, US_ASCII);

						String paramStr = idx + "," + branchID + "," + gmmID + LF;
	...
	private static void initBranchData(String srcPath, 
		List<String> branchIDs, List<Double> branchWTs) {
		
		File firstSrcDir = new File(srcPath, "NSHMP14-AS");
	...
	private static void initOutputDirsAndFiles(String outPath, 
			Set<Period> periods, Set<String> locIDs) throws IOException {
		for (Period p : periods) {
			for (String locID : locIDs) {
				...
				Iterable<String> gmVals = Collections2.transform(
					p.getFunction().xValues(), Functions.toStringFunction());
				...
				// write header
				Files.write("BranchIndex,BranchName,GMM" + LF, pFile, US_ASCII);
			}
		}
	}
	...
	private static void createFlagFile(String branchPath, String flagPath)
		...
		for (String line : Iterables.skip(branchLines, 1)) {
			...
			branchNames.add(Iterables.get(vals, 1) + "_" + Iterables.get(vals, 2));
		}
		...
		// logic tree identifiers to strings so we can incorporate gmm branches
		...
		for (String branchName : branchNames) {
			int _idx = branchName.lastIndexOf('_');
			// UC3 logic tree branches
			String branchID = branchName.substring(0, _idx);
			...
			for (Class<? extends LogicTreeBranchNode<?>> clazz : UC33classList) {
				...
				nodeTable.put(branchName, node, true);
			}
			// GMMs
			String gmmID = branchName.substring(_idx + 1);
			nodeTable.put(branchName, gmmID, true);
		}
