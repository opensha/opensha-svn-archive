...
public class GMT_MapFromGriddedHypoMFD_Forecast {
	...
	public void makeMap(double mag, String dirName, boolean isAdjustLatLon, double adjustmentVal) {
		...
		CPTParameter cptParam =
			(CPTParameter)mapGenerator.getAdjustableParamsList().getParameter(GMT_MapGenerator.CPT_PARAM_NAME);
	...
	public  GeoDataSet getRELM_XYZ_DataAboveMag(double mag, boolean isAdjustLatLon, double adjustment) {
		...
		for(int i=0; i<numLocs; ++i) {
			...
			double lat, lon;
			if(isAdjustLatLon) {
				lat = loc.getLatitude()-adjustment;
				lon = loc.getLongitude()-adjustment;
			} else {
				lat = loc.getLatitude();
			...
			}catch(IllegalArgumentException dataPointException) {
			...
			xyzDataSet.set(new Location(lat, lon), rateAboveMag);
		}
