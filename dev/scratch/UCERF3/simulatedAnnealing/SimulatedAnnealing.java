...
public class SimulatedAnnealing {
	...
	private NonnegativityConstraintType nonnegativeityConstraintAlgorithm;
	...
	private void setup(DoubleMatrix2D A, double[] d, double[] initialState) {
		...
		coolingFunc = CoolingScheduleType.VERYFAST_SA;
		nonnegativeityConstraintAlgorithm = NonnegativityConstraintType.TRY_ZERO_RATES_OFTEN;
	...
	public synchronized void iterate(int numIterations) {
		iterate(0, numIterations);
	}

	protected synchronized void iterate(int startIter, int numIterations) {
...
//		long setupTime = System.currentTimeMillis();
		...
		int totIterations = startIter + numIterations;

		for (iter = startIter+1; iter <= totIterations; iter++) {
