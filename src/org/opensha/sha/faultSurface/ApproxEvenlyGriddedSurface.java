...
public class ApproxEvenlyGriddedSurface extends EvenlyGriddedSurface {
	...
	public ApproxEvenlyGriddedSurface(FaultTrace upperFaultTrace,FaultTrace lowerFaultTrace, double aveGridSpacing) {
		...
		Location firstUpperLoc = upperFaultTrace.getLocationAt(0);
		Location firstLowerLoc = lowerFaultTrace.getLocationAt(0);
		Location lastLowerLoc = lowerFaultTrace.getLocationAt(lowerFaultTrace.size()-1);
		double firstFirstDist = RelativeLocation.getApproxHorzDistance(firstUpperLoc, firstLowerLoc);
		double firstLastDist = RelativeLocation.getApproxHorzDistance(firstUpperLoc, lastLowerLoc);
		...
		double dipDir = RelativeLocation.getAzimuth(upperFaultTrace.getLocationAt(0), lowerFaultTrace.getLocationAt(0));
		...
		for(int i=0; i<resampUpperTrace.size(); i++) {
			Location topLoc = resampUpperTrace.getLocationAt(i);
			Location botLoc = resampLowerTrace.getLocationAt(i);
		...
		for(int c=0; c<resampUpperTrace.size(); c++) {
			Location topLoc = resampUpperTrace.getLocationAt(c);
			...
			for(int r=1;r<nRows;r++) {
				Location nextLoc = RelativeLocation.getLocation(prevLoc, dir);
		...
		Location loc1 = resampLowerTrace.getLocationAt(resampLowerTrace.size()-1);
		...
		System.out.println(resampLowerTrace.getLocationAt(resampLowerTrace.size()-1).toString());
		System.out.println(resampUpperTrace.getLocationAt(resampLowerTrace.size()-1).toString());
	...
	public void test() {
		...
		double accuracyCheck = RelativeLocation.getTotalDistance(botLoc, prevLoc);
		System.out.println("Distance between actual and computed bottom point = "+(float)accuracyCheck);
		System.out.println("DeltaLat = "+ (float)((botLoc.getLatitude()-prevLoc.getLatitude())*111));
		System.out.println("DeltaLon = "+ (float)((botLoc.getLongitude()-prevLoc.getLongitude())*111));
		System.out.println("DeltaDepth = "+ (float)(botLoc.getDepth()-prevLoc.getDepth()));
	...
	public double computeAveGridSpacingAlongStrike() {
		...
		for(int r=0; r<numRows;r++)
			for(int c=0; c<numCols-1; c++) {
				aveDist += RelativeLocation.getTotalDistance(getLocation(r, c), getLocation(r, c+1));
				num +=1;
			}
		return aveDist/num;
	}
	...
	public double computeAveGridSpacingDownDip() {
		...
		for(int c=0; c<numCols; c++)
			for(int r=0; r<numRows-1;r++) {
				aveDist += RelativeLocation.getTotalDistance(getLocation(r, c), getLocation(r+1, c));
				num +=1;
			}
		return aveDist/num;
	}
