...
public class PaleoSitePublicationsDB_DAO {
  ...
  public void addPaleoSitePublicationInfo(PaleoSitePublication paleoSitePublication) {
    ...
    ArrayList siteTypeNames = paleoSitePublication.getSiteTypeNames();
     try {
       for(int i=0; i<siteTypeNames.size(); ++i) {
         int siteTypeId = this.siteTypeDAO.getSiteType((String)siteTypeNames.get(i)).getSiteTypeId();
   ...
   * @return
   */
  public ArrayList getPaleoSitePublicationInfo(int siteId) {
  ...
  private ArrayList query(String condition) {
    ...
    String sql =  "select "+SITE_ID+",to_char("+SITE_ENTRY_DATE+") as "+SITE_ENTRY_DATE+","+
        ...
        SITE_TYPE_ID+","+REPRESENTATIVE_STRAND_INDEX+","+this.CONTRIBUTOR_ID+","+
        REFERENCE_ID+" from "+this.TABLE_NAME+condition;
    HashMap refIdPublicationMap = new HashMap();
    try {
      ResultSet rs  = dbAccess.queryData(sql);
      int estId;
      while(rs.next())  {
        int referenceId = rs.getInt(this.REFERENCE_ID);
        ...
        if (refIdPublicationMap.containsKey(new Integer(referenceId))) {
          ...
          paleoSitePublication.setContributorName(contributorName);
          paleoSitePublication.setEntryDate(rs.getString(ENTRY_DATE));
          paleoSitePublication.setRepresentativeStrandName(this.siteRepresentationDAO.getSiteRepresentation(rs.getInt(this.REPRESENTATIVE_STRAND_INDEX)).getSiteRepresentationName());
