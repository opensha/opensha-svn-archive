...
public abstract class AbstractParameter<E> implements Parameter<E> {
	 ...
	 */
	public AbstractParameter( String name, ParameterConstraint constraint, String units, E value )
	...
	public Element toXMLMetadata(Element root, String elementName) {
		...
		if (this instanceof Parameter) {
			...
			if (num > 0) {
				Element dependent = xml.addElement(Parameter.XML_INDEPENDENT_PARAMS_NAME);
				ListIterator<Parameter> it = (ListIterator<Parameter>)param.getIndependentParametersIterator();
				while (it.hasNext()) {
...
//		System.out.println("success? "+success);
	...
	public Parameter getIndependentParameter(String name) throws ParameterException {
		...
		if( index != -1 ) {
			Parameter<?> param = (Parameter<?>)independentParameters.get(index);
			return param;
		}
		else{
	...
	private int getIndexOf(String paramName){
		...
		for(int i=0;i<size;++i){
			if(((Parameter<?>)independentParameters.get(i)).getName().equals(paramName))
				return i;
		}
		return -1;
	}
