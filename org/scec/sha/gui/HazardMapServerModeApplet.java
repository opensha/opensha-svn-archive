...
public class HazardMapServerModeApplet extends JApplet implements
  ...
  public HazardMapServerModeApplet() {
  }
  ...
  public static void main(String[] args) {
    HazardMapServerModeApplet applet = new HazardMapServerModeApplet();
  ...
  public void computeHazardCurve() {
   ...
   try {
     ...
     //opens the connection with the server to do the calculations
     openConnection(condProbFunc,hazFunction,griddedRegionSites,imr,eqkRupForecast);
  ...
  public String getMapParametersInfo(){
    return "IMR Param List: \n" +
        ...
        "Site Param List: \n"+
        "\t\t"+griddedRegionGuiBean.getGriddedRegionParameterListEditor().getParameterList().toString()+"\n"+
        "IMT Param List: \n"+
...
/**
  ...
  */
 void openConnection(ArbitrarilyDiscretizedFunc condProbFunc,ArbitrarilyDiscretizedFunc hazFunc,
                     SitesInGriddedRegion regionSites,AttenuationRelationshipAPI imr,
   ...
   int size = condProbFunc.getNum();

   //adding the X values(log of actaul X values) for the condProbfunc to the Servlet
   for(int i=0;i<size;i++)
     condProbVector.add(new Double(condProbFunc.getX(i)));

   for(int i=0;i<size;i++)
     hazFuncVector.add(new Double(hazFunc.getX(i)));

   try{
     if(D) System.out.println("starting to make connection with servlet");
     URL hazardMapServlet = new
                           URL("http://scec.usc.edu:9999/examples/servlet/HazardMapCalcServlet");
     ...
     outputToServlet.writeObject(regionSites);
