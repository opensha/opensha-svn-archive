...
public class IMRGuiList {
    ...
    private IMRGuiBean currentGui = null;
     ...
     */
    public IMRGuiBean setImr( String imrName, IMRTesterApplet applet ) {

        if ( ( currentGui != null )  && ( currentGui.getName().equals( imrName ) ) )  return currentGui;
        else if ( imrGuis.containsKey( imrName ) ) {
            currentGui = ( IMRGuiBean ) imrGuis.get( imrName );
            return currentGui;
        }
        else {

            String className = IMRTesterApplet.imrNames.get( imrName ).toString();
            IMRGuiBean bean = new IMRGuiBean( className, imrName, applet );
